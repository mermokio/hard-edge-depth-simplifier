...
export async function decodeDepthBlob(source:Blob,maxDimension:number,name="Depth map"):Promise<DepthImage>{
  if(!/^image\/(png|jpeg|webp)$/.test(source.type))throw new Error("Choose a PNG, JPEG, or WebP image.");
  let bitmap:ImageBitmap;try{bitmap=await createImageBitmap(source);}catch{throw new Error("The image could not be decoded.");}
  ...
  if(!count)throw new Error("The image is completely transparent.");return{width:w,height:h,depth,valid,name};
}

export const decodeDepthImage=(file:File,maxDimension:number)=>decodeDepthBlob(file,maxDimension,file.name);
