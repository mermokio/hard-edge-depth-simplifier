...
function pruneSupport(labels:Int32Array,valid:Uint8Array,w:number,h:number,bh:Uint8Array,bv:Uint8Array,s:number,minFraction:number){
  ...
  for(let y=0;y<h;y++)for(let x=0;x<w-1;x++){const i=y*(w-1)+x;if(!bh[i])continue;const a=labels[y*w+x],b=labels[y*w+x+1],z=test(x+.5,y,a,b);supportH[i]=z;if(z<minFraction){outH[i]=0;removedH[i]=1;}}
  for(let y=0;y<h-1;y++)for(let x=0;x<w;x++){const i=y*w+x;if(!bv[i])continue;const a=labels[y*w+x],b=labels[(y+1)*w+x],z=test(x,y+.5,a,b);supportV[i]=z;if(z<minFraction){outV[i]=0;removedV[i]=1;}}
...
function mergeSmall(labels:Int32Array,count:number,img:DepthImage,normals:Float32Array,p:Parameters){
  ...
  for(let iter=0;iter<8;iter++){iterations=iter+1;const info=regionInfo(current,currentCount,img,normals);if(!sources.length)sources=info.map(()=>1);const small=info.map(I=>2*I.maxDist<p.minimumDiameterFactor*p.minimumScale||I.area<p.minimumAreaFactor*p.minimumScale*p.minimumScale);if(iter===0){belowTotal=small.filter(Boolean).length;for(let i=0;i<current.length;i++)if(current[i]>=0&&small[current[i]])belowMask[i]=1;}if(!small.some(Boolean)){converged=true;break;}const dest=new Int32Array(currentCount).map((_,i)=>i);let changed=0;
    for(let a=0;a<currentCount;a++)if(small[a]&&info[a].neighbors.size){let best=-1,cost=Infinity;for(const [b,shared]of info[a].neighbors){if(b<0||b>=currentCount)continue;const A=info[a],B=info[b],c=p.mergeNormalWeight*(angle(...A.normal,...B.normal)/Math.PI)+p.mergeDepthWeight*Math.abs(A.meanDepth-B.meanDepth)-shared/Math.max(1,A.perimeter);if(c<cost){cost=c;best=b;}}if(best>=0){dest[a]=best;changed++;merges++;}}
...
export function runPipeline(input:DepthImage,p:Parameters):PipelineResult{
  ...
  const comps=edgeComponents(qh.raw,qv.raw,input.width,input.height,p.minimumBoundaryLengthFactor*p.minimumScale);mark("edge length");const initial=labelRegions(input.valid,input.width,input.height,comps.keepH,comps.keepV);const support=pruneSupport(initial.labels,input.valid,input.width,input.height,comps.keepH,comps.keepV,p.minimumScale,p.minimumSideSupportFraction);const supported=labelRegions(input.valid,input.width,input.height,support.outH,support.outV);mark("scale support");
  ...
  const violations=rec.stats.filter(x=>x.scaleStatus==="violation").length;const result:PipelineResult={width:input.width,height:input.height,depth,valid:input.valid,smoothed,normals,depthChangeH:c.dh,depthChangeV:c.dv,normalChangeH:c.ah,normalChangeV:c.av,combinedH:c.ch,combinedV:c.cv,concentrationH:qh.q,concentrationV:qv.q,rawH:qh.raw,rawV:qv.raw,retainedH:fb.H,retainedV:fb.V,removedShortH:comps.removedH,removedShortV:comps.removedV,removedSupportH:support.removedH,removedSupportV:support.removedV,supportH:support.supportH,supportV:support.supportV,initialLabels:initial.labels,belowScaleMask:merged.belowMask,mergeTargets:merged.mergeTargets,finalLabels:merged.labels,meanNormalMap:rec.meanNormalMap,reconstruction:rec.bounded,unclippedReconstruction:rec.raw,outOfRange:rec.oor,residual:rec.residual,absResidual:rec.absResidual,stats:rec.stats,audit:{minimumScale:p.minimumScale,minimumEdgeLength:p.minimumBoundaryLengthFactor*p.minimumScale,minimumRegionDiameter:p.minimumDiameterFactor*p.minimumScale,minimumRegionArea:p.minimumAreaFactor*p.minimumScale*p.minimumScale,rawComponents:comps.components,removedShortComponents:comps.removed,removedUnsupportedLinks:support.removed,provisionalRegions:initial.count,belowScaleRegions:merged.belowTotal,mergeOperations:merged.merges,finalRegions:merged.count,finalViolations:violations,converged:merged.converged,iterations:merged.iterations},timings,totalMs:now()-t0};return result;
}
