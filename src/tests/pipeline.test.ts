...
describe("scale-constrained hard-edge pipeline",()=>{
  ...
  it("forms partitions from aggregate interfaces when raw edge rows contain gaps",()=>{const img=make(128,72,(u,v)=>{const row=Math.round(v*71);if(row%15===0&&Math.abs(u-.5)<.03)return .2+.55*(u-.47)/.06;return u<.5?.2:.75;}),r=runPipeline(img,{...params,minimumScale:12,microRegionSize:10});let gaps=0;for(let y=0;y<72;y++)if(!r.rawH[y*127+63])gaps++;expect(gaps).toBeGreaterThan(3);expect(r.audit.finalRegions).toBeGreaterThanOrEqual(2);});
